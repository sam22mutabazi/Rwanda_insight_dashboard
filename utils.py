...
# --- FIX FOR FORECAST LOGIC ---
def run_forecast_logic(df):
	"""
    Handles the prediction logic for the Trends page.
    Ensures that if 'Age Group' is missing, it falls back to 'Gender'.
    """
	...
	if 'Age Group' in df.columns:
		...
		trend['Forecast'] = (trend['Current'] * 1.25).astype(int)  # Predicting 25% growth
		return trend

	# Fallback to Gender if Age Group is missing
	if 'Gender' in df.columns:
		trend = df.groupby('Gender').size().reset_index(name='Current')
		trend['Forecast'] = (trend['Current'] * 1.15).astype(int)  # Predicting 15% growth
		return trend

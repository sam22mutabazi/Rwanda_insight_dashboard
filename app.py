...
with st.sidebar:
	...
	if st.session_state.get("is_admin", False): nav.append("🔑 Admin Panel")
...
if uploaded_file:
	...
	# 4. HIERARCHICAL FILTERING (District -> Sector -> Cell)
	filtered_df = df.copy()
	loc_label = "All Rwanda"
	...
	if sel_dist != "All Districts":
		...
		if sel_sect != "All Sectors":
			filtered_df = filtered_df[filtered_df['Sector'] == sel_sect]
			loc_label = f"{sel_dist} - {sel_sect}"

			cell_list = ["All Cells"] + sorted(
				filtered_df['Cell'].dropna().unique().tolist()) if 'Cell' in filtered_df.columns else ["All Cells"]
			sel_cell = st.sidebar.selectbox("Select Cell", cell_list)

			if sel_cell != "All Cells":
				filtered_df = filtered_df[filtered_df['Cell'] == sel_cell]
				loc_label = f"{sel_sect} - {sel_cell}"
	...
	# --- PAGE ROUTING ---
	if page == "📊 Analytics":
		st.title(f"🏛️ {loc_label} Analytics")
		...
		with t1:
			...
			st.plotly_chart(
				px.histogram(filtered_df, x='District' if 'District' in filtered_df.columns else df.columns[0],
							 ...
							 title="Participation Trends"), use_container_width=True)
		...
		with t2:
			...
			# FIXED: Now sends both filtered_df and loc_label to match utils.py
			c_ex2.download_button("📄 PDF Report", generate_pdf_report(filtered_df, loc_label),
								  f"Report_{loc_label}.pdf")

			st.dataframe(filtered_df.style.highlight_null(color='#ff4b4b'), use_container_width=True)

		with t3:
			if 'Sector' in filtered_df.columns:
				for s in filtered_df['Sector'].unique()[:5]: st.date_input(f"Date for {s}", datetime.date(2026, 5, 1),
																		   key=s)
		...
		if 'District' in df.columns:
			...
			d2 = st.selectbox("District B", df['District'].unique(), key="comp_b",
							  index=min(1, len(df['District'].unique()) - 1))
			...
			st.error("No District column available.")
		...
		if history_data:
			st.plotly_chart(
				px.line(pd.DataFrame(history_data), x="date", y="count", title="System Usage Trends", markers=True),
				use_container_width=True)
		...
		if st.button("🚀 Run Forecast"):
			...
			if trend_data is not None:
				...
				st.plotly_chart(px.line(trend_data, x=category_col, y=['Current', 'Forecast'],
										title=f"Prediction by {category_col}", markers=True), use_container_width=True)
...
if page == "🔑 Admin Panel":
	if not st.session_state.get("is_admin", False):
		...
		tab1, tab2, tab3 = st.tabs(["👥 Users", "📜 Logs", "🛡️ System"])
		with tab1:
			users = load_users()
			st.dataframe(pd.DataFrame([{"Username": k, "Admin": v["is_admin"]} for k, v in users.items()]),
						 use_container_width=True)
			target_user = st.selectbox("Select User", list(users.keys()))
			if st.button("Toggle Admin Role"):
				if toggle_admin_role(target_user, st.session_state.user): st.rerun()
			if st.button("❌ Delete Account"):
				if delete_user(target_user): st.rerun()
		with tab2:
